...
NSString * GeofencingClassic$ModuleRequireFormat = @"(function(exports){"
...
@implementation GeofencingClassicObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_geofencingclassic);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create GeofencingClassic global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "GeofencingClassic" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_geofencingclassic = [[GeofencingClassicObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_geofencingclassic];
	
	NSString *_geofencingclassicNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _geofencingclassicNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_geofencingclassic valueForKey:name];
...
{
	NSString *js = [[NSString alloc] initWithFormat:GeofencingClassic$ModuleRequireFormat,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.geofencingclassic.kroll" 
	...
	@throw [NSException exceptionWithName:@"org.geofencingclassic.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
