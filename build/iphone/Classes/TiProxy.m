...
//Rare exceptions to indicate a bug in the _geofencingclassic code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_geofencingclassic = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_geofencingclassic]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.geofencingclassic.module"
